...
trait Transformer extends Runnable {this: Command =>
  ...
  protected val timeout        = option[Seconds]("-t" :: "--timeout" :: Nil, "limited trace seconds.", 10)
  ...
  protected lazy val probeTransformer = new ClassFileTransformer {
    ...
    def transform(
      ...
      val filter: ClassDecorator.MethodFilter = (c: String, s: String, i: Array[String], m: String) => {
        (methodFilters() map {_.lazyFilter(c, s, i)}).find { f => f(m) }.isDefined
      }

      try {
        if (candidates.contains(classBeingRedefined)) ClassDecorator.decorate(classfileBuffer, filter)
        else null
      } catch {
